...
module EventStore =
    ...
    let processEvents (message: StreamData<_, _>) =
        task {
            let lastVersion, metadataEntity =
                let res =
                    message.StreamStore.ContainsKey(message.StreamId)
                    |> fun x ->
                        if x then Some message.StreamStore.[message.StreamId]
                        else None
                match res with
                | Some r ->
            ...
            ops
            |> List.map (fun x -> message.EventStore.TryAdd(x.Id, x))
            |> List.fold (fun acc x -> x && acc) true
            |> ignore
            return ops
        }
    ...
    let private appendEvents (streamStore) (eventStore) (streamId: string) (expectedVersion: ExpectedVersion<_>)
    ...
    let private getEvents (store: EventStoreType<_, _>) (streamId: string) (eventsRead: EventsReadRange<_>) =
        task {
            let fetch =
    ...
    let private getEvent store streamId version =
        task {
    ...
    let private getStreams (store: StreamStoreType<_>) streamsRead =
        task {
            let res =
                ...
                | Contains c ->
                    store.Keys
                    |> Seq.filter (fun x -> x.Contains(c))
                    |> Seq.map (fun x -> store.[x])
                    |> Seq.toList
                | EndsWith c ->
                    store.Keys
                    |> Seq.filter (fun x -> x.EndsWith(c))
                    |> Seq.map (fun x -> store.[x])
                    |> Seq.toList
                | StartsWith c ->
                    store.Keys
                    |> Seq.filter (fun x -> x.StartsWith(c))
                    |> Seq.map (fun x -> store.[x])
                    |> Seq.toList
            return (res
    ...
    let private getStream (store: StreamStoreType<_>) streamId =
        task {
